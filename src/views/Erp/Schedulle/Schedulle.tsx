...
const Schedulle = () => {
  ...
  const getWeather = (value: Date) => {
    switch (value.getDay()) {
      case 0:
        return '<img class="weather-image"  src= "assets/images/13.png" />';
      case 1:
        return '<img class="weather-image" src= "assets/images/13.png" />';
      case 2:
        return '<img class="weather-image" src= "assets/images/13.png" />';
  ...
  const dateHeaderTemplate = (props: { date: Date; }) => {
    return (
      <Fragment>
        ...
        <div className="date-text" dangerouslySetInnerHTML={{ __html: getWeather(props.date) as string }}></div>
      </Fragment>
    );
  }
  ...
  let generateEvents = (): Record<string, any>[] => {
    ...
    if (Browser.isIE) {
      Timezone.prototype.offset = (date: Date, timezone: string): number => tz.zone(timezone)?.utcOffset(date.getTime()) as number;
    }
  ...
  const contextMenuSelect = (args: ContextMenuEventArgs) => {
    ...
    switch (selectedMenuItem) {
      ...
      case 'Add':
      case 'AddRecurrence':
        let selectedCells: Element[] = scheduleObj.current?.getSelectedElements() as Element[];
        let activeCellsData: CellClickEventArgs = scheduleObj.current?.getCellDetails(selectedCells.length > 0 ? selectedCells : selectedTarget) as CellClickEventArgs;
  ...
  return (
    <div className='schedule-control-section' style={{width: '100%'}}>
      <div className='col-lg-12 control-section'>
        <div className='content-wrapper'>
          <div className='schedule-overview'>
            ...
            <ToolbarComponent id='toolbarOptions' cssClass='overview-toolbar' width='100%' height={70} overflowMode='Scrollable' scrollStep={100} created={() => liveTimeInterval = 0 /*setInterval(() => { updateLiveTime(); }, 1000)*/} clicked={onToolbarItemClicked}>
              <ItemsDirective>
