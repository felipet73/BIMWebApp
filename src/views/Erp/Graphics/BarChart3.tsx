...
const BarChart3 = () => {
    ...
    const loaded = (args: ILoadedEventArgs): void => {
        let chart: Element = document.getElementById('charts3') as Element;
    ...
    return (
        <div className='control-pane'>
            ...
            <div className='control-section'>
                <ChartComponent id='charts3' style={{ textAlign: "center" }} enableSideBySidePlacement={false} primaryXAxis={{ valueType: 'Category', interval: 1, majorGridLines: { width: 0 }, majorTickLines: {width : 0}, minorTickLines: {width: 0}, labelPosition: 'Outside', labelRotation: Browser.isDevice ? -45 : 0, labelIntersectAction : Browser.isDevice ? 'None' : 'Rotate45' }} primaryYAxis={{ minimum: 0, maximum: 100, title: 'Literacy Rate In Percentage', labelFormat: '{value}%', interval: 25, majorTickLines: { width: 0 }, lineStyle: { width: 0 } }} chartArea={{ border: { width: 0 } }} highlightColor='transparent' load={load.bind(this)} title='Example graphic' loaded={loaded.bind(this)} legendSettings={{ visible: false }} width={Browser.isDevice ? '100%' : '77%'} tooltip={{ enable: true, header: "<b>${point.x}</b>", format: "Rate : <b>${point.text}</b>" }} pointRender={pointRender}>
