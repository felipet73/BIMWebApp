...
const TreeDocumentsIssue = () => {
  ...
  /*useEffect(() => {
    ...
    if (modeDetails === 'ProjectViews') {
      console.log('Model actual',actualModel);
        ...
        const ObteinDataFolderViews = (async ()=>{
          ...
          auxData.current = data.folderviews.filter( (y:any) => y.model===actualModel.id).map( (folder:any) => ( { ...folder, type:'folders', children: [ ...folder.views.map( (x:any) => ({...x,type:'view'}) ) ] } ))
                  ...
                  return (
                    {
                      name:x.name,
                      type:'category',
                      children: data.folderviews.filter( (y:any) => y.model===x.id).map( (folder:any) => ( { ...folder, type:'folders', children: folder.views } ) )
                    }
                  )
                })
              ]
